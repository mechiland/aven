...
<head>
  ...
  <link rel="preload" href="/assets/aven-files.png" as="image"><script src="/app.js" defer></script>
</head>
<body>
...
<main id="main">
  <section class="hero" aria-labelledby="hero-title">
    <div class="hero-copy wrap">
      ...
      <a class="release-line" href="${release}">${c.version}${chevron}</a>
    </div>
  ...
  <section class="everyday section" id="everyday" aria-labelledby="daily-title">
    ...
    <div class="experience wrap">
      ...
      ${scenes.map((s, i) => `<div class="experience-panel" role="tabpanel" id="panel-${s.id}" aria-labelledby="tab-${s.id}" tabindex="0" ${i === 0 ? '' : 'hidden'}>
        <div class="experience-copy"><div><span class="app-name">${s.app}</span><h3>${c.sceneTitle[i]}</h3></div><p>${c.sceneText[i]}</p></div>
        <a class="screenshot-link" data-lightbox href="/assets/aven-${s.id}.png" aria-label="${c.screenshot} — ${c.labels[i]}"><img src="/assets/aven-${s.id}.png" alt="Aven · ${c.labels[i]} — ${c.sceneTitle[i]}" width="1920" height="1200" loading="lazy" decoding="async"><span class="expand-image">${expand}</span></a>
      </div>`).join('')}
    ...
    <div class="wrap">
      ...
      <figure class="compare-figure">
        <div class="compare-stage" style="--split:50%">
          <img class="compare-after" id="after-image" src="/assets/aven-files.png" alt="Aven · ${c.labels[0]}" width="1920" height="1200" loading="lazy">
          ...
          <input type="range" class="compare-range" id="compare-range" min="0" max="100" value="50" aria-label="${c.slider}" aria-valuetext="${c.before} 50%, Aven 50%">
        </div>
        <div class="compare-toolbar"><a id="stock-original" href="/assets/stock-files.png" data-lightbox>${c.before} ${expand}</a><span class="drag-hint">↔ ${c.drag}</span><a id="aven-original" href="/assets/aven-files.png" data-lightbox>Aven ${expand}</a></div>
  ...
  <section class="typography section" id="typography" aria-labelledby="type-title">
    ...
    <div class="type-specimen"><span class="specimen-label">Aa <span>字</span></span><p class="specimen-chinese" lang="zh-CN">${c.typeSample}</p><p class="specimen-english" lang="en">${c.typeEnglish}</p><div class="specimen-bottom"><span>${c.typeMeta}</span><span>Noto Sans</span></div></div></div>
  </section>
  ...
  <section class="download section" id="download" aria-labelledby="download-title"><div class="wrap">
    ...
    <div class="download-actions"><a class="button" href="${release}">${c.download}${arrow}</a><a class="text-link" href="${docs}/ISO.md">${c.guide}${chevron}</a></div><p class="platform">${c.platform}</p><p class="prerelease">${c.prerelease}</p>
    <details class="install-details"><summary>${c.installTitle}<span aria-hidden="true">+</span></summary><ol>${c.installSteps.map(s => `<li>${s.replaceAll('python3 reassemble-iso.py', '<code>python3 reassemble-iso.py</code>').replaceAll('py -3 reassemble-iso.py', '<code>py -3 reassemble-iso.py</code>')}</li>`).join('')}</ol><p>${c.installNote}</p></details>
  </div></section>
</main>

<footer><div class="wrap">
  <div class="footer-top"><a class="wordmark" href="/${lang}/">aven</a><p>${c.footerLine}</p></div>
  <div class="evidence-note"><p><strong>${c.evidenceTitle}${lang === 'en' ? '.' : '。'}</strong> ${c.evidenceText}</p><p>${c.limits}</p><div class="evidence-links"><a href="${docs}/ISO-VERIFICATION.md">${c.evidence}</a><a href="${docs}/critic-iso-candidate5.md">${c.critic}</a><a href="/credits.txt">${c.credits}</a></div></div>
  <div class="footer-bottom"><span>© 2026 ${c.copyright}</span><div><a href="${repository}">${c.footerSource}</a><a href="${release}">${c.releaseNotes}</a></div><div class="footer-languages" aria-label="${c.languageLabel}">${globe}<a href="/zh-cn/" lang="zh-CN" ${lang === 'zh-cn' ? 'aria-current="page"' : ''}>简体中文</a><span>/</span><a href="/en/" lang="en" ${lang === 'en' ? 'aria-current="page"' : ''}>English</a></div></div>
</div></footer>
...
<script type="application/json" id="scene-data">${JSON.stringify({ids: scenes.map(s => s.id), labels: c.labels, details: c.compareDetails, before: c.before, screenshot: c.screenshot}).replaceAll('<', '\\u003c')}</script>
</body></html>`;
}
